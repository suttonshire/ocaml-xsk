...
let mac_of_string str =
  str
  |> String.split ~on:':'
  |> fun l -> if List.length l <> 6 then raise (Invalid_argument ("invalid mac address")) else l
...
let ip_of_string str =
  str
  |> String.split ~on:'.'
  |> fun l -> if List.length l <> 4 then raise (Invalid_argument ("invalid ip address")) else l
  |> List.map ~f:Int.of_string
  |> List.map ~f:Char.of_int_exn
  |> String.of_char_list
;;
      ...
      *)
      ...
      let sent =
        ...
        then sent + send_batch tx_batch_size frame_size sent txq fd descs
          ...
          sent)
      in
      let consumed0 = Xsk.Comp_queue.consume cq addrs ~pos:0 ~nb:max_batch_size
      in
    ...
    Command.Let_syntax.(
      ...
      and frame_size =
        flag
          ...
          ~doc:"n Size of each frame in the umem"
      and src_ip =
        flag
          "-sip"
      ...
      and dst_mac =
        flag
          "-dmac"
          (required string)
          ~doc:"destination_mac Destination mac address to use in the ethernet headers. Format XX:XX:XX:XX:XX:XX"
      and zero_copy =
      ...
      in
      let xdp_flags = match zero_copy with
      | None -> make_xdp_flags None
      | Some _ -> make_xdp_flags (Some Xsk.Xdp_flag.XDP_FLAGS_DRV_MODE) in
